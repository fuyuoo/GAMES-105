...
#     path_positions, path_orientations = []
#     # init path_offset
#     # init path_ori
...
#     #         joint_rotations.append(
#     #             R.from_quat(joint_orientations[parent_index]).inv() * R.from_quat(joint_orientations[i]))
#
#     # init offset
...
#     cnt = 0
#     end_index = joint_names.index(meta_data.end_joint)
#     while min_distance > 0.01 and cnt <= 10:
#         for i in range(len(path) - 2, -1, -1):
#             index = path[i]
#             cur_position = joint_positions[index]
#             target_vector = target_pose - cur_position
#             end_vector = joint_positions[end_index] - cur_position
#
#             # 计算旋转向量
#             rotation_axis = np.cross(target_vector, end_vector)
#             rotation_axis /= np.linalg.norm(rotation_axis)
#             cos_angle = np.dot(target_vector, end_vector) / (np.linalg.norm(target_vector) * np.linalg.norm(end_vector))
#             angle_rad = np.arccos(np.clip(cos_angle, -1.0, 1.0))
#             rotation_vector = angle_rad * rotation_axis
#             joint_orientations[index] = (R.from_euler("XYZ", rotation_vector) * R.from_quat(joint_orientations[index])).as_quat()
#
#             # 计算local rotation
#             path_rotation = []
#             for j in range(len(path)):
#                 if j == 0:
#                     rotation_save = R.from_quat(joint_orientations[path[j]])
#                 else:
#                     parent_index = path[j - 1]
#                     rotation_save = R.inv(R.from_quat(joint_orientations[parent_index])) * R.from_quat(joint_orientations[j])
#                 path_rotation.append(rotation_save)
#             # 局部FK
#             for j in range(i, len(path) - 1):
#                 j_parent_index = path[j]
#                 j_index = path[j + 1]
...
#                 else:
#                     joint_orientations[j_index] = (R.from_quat(joint_orientations[j_parent_index]) * path_rotation[j]).as_quat()
#
#             min_distance = min(min_distance, np.linalg.norm(target_pose - joint_positions[end_index]))
